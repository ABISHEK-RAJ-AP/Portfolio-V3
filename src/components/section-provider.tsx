...
import { SECTION_THEME_MAP, SECTIONS, WORK_SUBSECTIONS } from '@/lib/constants'; // Add this import
import { Direction, SectionColors, SectionContextType, Sections } from '@/lib/types';
import React, { createContext, useCallback, useEffect, useMemo, useState } from 'react';

export const SectionContext = createContext<SectionContextType | undefined>(undefined);

// Use useMemo to safely initialize constants when required
export const SectionProvider = ({ children }: { children: React.ReactNode }) => {
  const ANIMATION_DURATION_MEMO = useMemo(() => ({
    SHORT: 0.2,
    MEDIUM: 0.4,
    LONG: 0.8,
  }), []);

  const TRANSITION_DURATION = ANIMATION_DURATION_MEMO.MEDIUM * 3000;
  ...
  const [navigationDirection, setNavigationDirection] = useState<Direction>('down');
